...
// ─── Tauri IPC Bridge ───
// In Tauri desktop, fetch is replaced by invoke('search', ...) and invoke('analyze', ...)
// When running in browser (PWA), fall back to fetch API
...
async function doSearch(query) {
    ...
    try {
        let data;
        if (IS_TAURI) {
            data = await tauriInvoke('search', { query: q, limit: 20 });
        } else {
...
async function analyzeSnippet(snippet) {
    ...
    try {
        let data;
        if (IS_TAURI) {
            data = await tauriInvoke('analyze', { snippet });
        } else {
...
async function doCmdSearch(query) {
    ...
    try {
        let data;
        if (IS_TAURI) {
            data = await tauriInvoke('search', { query: q, limit: 8 });
        } else {
...
async function loadStats() {
    try {
        if (IS_TAURI) {
            statsData = await tauriInvoke('stats', {});
        } else {
